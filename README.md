...
| `DIRECTOR_CODEX_SKILLS_DIR` | `~/.agents/skills` | where `install --codex` **and** `install --copilot` write the `$director-*` agent skills. One shared surface: both agents read this directory, either install provides it, and uninstalling one leaves it alone while the other remains |
...
| `DIRECTOR_HOOK_AGENT` | (unset) | which agent a hook is running under. **Not a knob to set:** `install --copilot` writes `DIRECTOR_HOOK_AGENT=copilot` into each hook command it registers, because Copilot's payloads carry nothing that identifies them (no transcript path), and Copilot needs a different output shape than Claude Code. Exporting it globally in your shell mis-flavors the other harnesses' hooks, since this value outranks payload detection: leave it to the installer. `director doctor` fails the Copilot row if the prefix goes missing, which is otherwise a silent loss of ground-truth injection |
