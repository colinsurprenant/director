...
Director makes the reset free, and you don't operate it: it wires into Claude Code and Codex through hooks, the session emits as it works, and that state is injected into the next one as ground truth. It moves you from **message bus** to **reviewer**, built around a shared, durable, **append-only event log** per repo:
