...
- **Release pipeline** — ✅ shipped via `.github/workflows/release.yml` (plain `go build` cross-compile matrix, darwin/linux/windows × amd64/arm64: tarballs, plus zipped `director.exe` for windows; published to GitHub Releases on tag push). A tag-time `windows-gate` job (build + `test -race` on `windows-latest`) blocks publish, so a release never ships what windows tests didn't pass. Remaining sub-item: a `curl|sh` installer (unix-likes, including WSL; native Windows would need its own path).
