...
- **Copilot CLI adapter** — ✅ core shipped as `director install --copilot` (one Director-owned hooks file at `~/.copilot/hooks/director.json`, plus the `$director-*` agent skills it shares with a Codex install; see `docs/specs/2026-08-17-copilot-adapter-design.md`). Registering the events under Claude Code's PascalCase names makes Copilot deliver the CC-dialect payload, so the UNCHANGED shims serve it too; the single divergence is the output shape (a flat `additionalContext`, selected by the `DIRECTOR_HOOK_AGENT=copilot` prefix the installer writes into every hook command). Because those shared shims are bash, the native-Windows install refusal below covers the `--copilot` target too. Remaining sub-items: an `events.jsonl` reader to light up the emit-guard and the context-fill handoff nudge on Copilot (both are explicitly skipped there today, gated on flavor rather than left to fail open on a transcript format they cannot read), and PowerShell hook entries, which Copilot's hooks file supports natively through a `powershell` sibling to the `bash` field and which could give Copilot a native-Windows path ahead of the item below.
- **Native Windows hook shims** — windows binaries ship and every manual verb (`emit`, `render`, `status`, `brief`, `show`, `resolve`, …) works from PowerShell, but the hook shims are bash scripts, so `director install` (every target: Claude Code, `--codex`, `--opencode`, whose plugin fallback tier is the unix-only install symlink, and `--copilot`) refuses on native Windows (guard in `cmd/director/installcmd.go`; uninstall stays available as a cleanup path) and the ambient layer — session-start injection, heartbeats, boundary nudges — is WSL-only. Shipping this means portable shims (PowerShell twins, or folding the shim logic into the binary) plus lifting the guard and updating the README/getting-started Windows notes.
