...
| **A row reads `abandoned` despite a fresh heartbeat** | Liveness also checks that the workstream's branch still exists. A branch that was merged away and deleted reads `abandoned` by design, so the cockpit self-cleans; rows without branch/dir info fail open and age out by TTL only. |
