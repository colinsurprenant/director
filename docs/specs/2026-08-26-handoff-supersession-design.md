...
An explicit handoff then retires **exactly the same-workstream positions its refs name**: nothing older, nothing newer. Retirement is set membership, not a mark. The union of those named ids across the workstream's explicit handoffs is its **superseded set**, recorded verbatim as `Projection.SupersededHandoffs`: the ids explicitly consumed by a later handoff's refs, which is one of the removal paths rather than all of them (the implicit mark and a conclusion retire positions too), recorded so explicit supersession stays observable. The implicit mark's removals need no list of their own: they are legible from the surviving stack itself.
...
The emit warning classifies against the log, not against the flags. Once the handoff is appended, `emit` reads the log back and sorts the new event into three cases: refs naming at least one same-workstream handoff below its own id is **explicit**, and says nothing; **no refs at all** is implicit and warns; **refs naming zero same-workstream handoffs** is implicit too and warns with its own wording, because that is the mis-copied-ULID shape (a note's id, a sibling workstream's position, some other line of the digest) that folds exactly like a ref-less handoff and that a flags-only check cannot see.

Both implicit shapes are warned only when there is something to lose: the workstream's resume stack as the fold saw it *before* this append. A genuinely first handoff, a workstream whose positions a completion note already concluded, and one whose positions were all superseded alike have nothing live below them, so an implicit handoff there buries nothing and the warning would be wallpaper — the same calibration the guard rule follows. The gate is the fold's own answer over the log minus this event, not a count of prior handoffs, and the exemption covers both shapes: mis-copied refs over a dead stack lose no more than a ref-less handoff does. That is what a flags-only check got wrong in both directions — it warned the correct first handoff, and it stayed silent on the mis-copied one.
...
| `director emit` | reads the log after appending a handoff and warns on stderr when the fold will read it as implicit (no refs, or refs naming no position of this workstream) *and* the workstream had a live resume stack to lose, and `--refs` help text carrying both reserved meanings |
