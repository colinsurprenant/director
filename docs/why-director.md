...
You don't operate Director; your agents do. It installs into Claude Code, Codex, OpenCode, and GitHub Copilot CLI as hooks, so the writing happens as sessions work and the state is there waiting when the next one starts. What it maintains is a shared, durable, **append-only event log** per repo (one static Go binary: no daemon, no database, no cloud, no telemetry), plus **deterministic projections** over it:
