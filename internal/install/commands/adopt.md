...
1. **Register (the deterministic floor).** Run `director adopt [<dir>]`. If it fails because the directory is not a git repository, relay its remedy (`git init` — an empty init is enough) and STOP; no analysis without git. Note the workstream id and CHARTER path it prints, and whether the CHARTER was freshly scaffolded or already present. Then run `director open-items` and keep the list — it is the dedupe set for step 5.
